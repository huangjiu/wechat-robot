...
public class WechatServiceImpl implements WechatService {
	 ...
	 */
	@Override
	public void handleMsg(User user, JSONObject data) {
		if (null == data) {
			return;
		}
		
		WechatMeta wechatMeta = user.getMeta();
		...
		for (int i = 0, len = AddMsgList.size(); i < len; i++) {
			...
			String name = getUserRemarkName( user , msg.getString("FromUserName"));
	...
	private String getUserRemarkName(User user , String id) {
		String name = "这个人物名字未知";
		for (int i = 0, len = user.getContact().getMemberList().size(); i < len; i++) {
			JSONObject member = user.getContact().getMemberList().get(i).asJSONObject();
	...
	public byte[] getImage(String uuid) {
		...
		HttpRequest.post(url, true, "t", "webwx", "_", DateKit.getCurrentUnixTime()).receive(byteOutputStream);
	...
	public String waitForLogin(String uuid) {
		...
		HttpRequest request = HttpRequest.get(url, true, "tip", 1, "uuid", uuid, "_", DateKit.getCurrentUnixTime());
		LOGGER.info("等待登录...");
		LOGGER.debug("" + request.toString());
		String res = request.body();
		request.disconnect();
	...
	@Override
	public void getRedirectInfo(WechatMeta wechatMeta) {
